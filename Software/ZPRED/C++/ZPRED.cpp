...
{	//string salt="783e007c783e007c";
	...
	bool VERBOSE=false,MULTI_DISPLAY=false,CRITICAL_COAGULATION;// boolean for controlling display updates beyond 150 threads
	int ch;
	...
	string hamakerFldr=fFldr+"Hamaker/"; if(!directory_exist(hamakerFldr)){make_folder(hamakerFldr);}					// Calculation Folder for Determining Hamaker Constant	
	//
	string pqr2pdbFile=currFldr+"saveAsPdb.py"; write_saveAsPdb(pqr2pdbFile);										// PyMol PQR2PDB Converter
	pqr2pdbFile=outputFldr+"saveAsPdb.py"; write_saveAsPdb(pqr2pdbFile);											// PyMol PQR2PDB Converter (Copy)
...
*/
	...
	for(int i=0;i<Z.numSolCond;i++)
		...
		Output+="Protein Relative Dielectric: "+Z.pdie+"\n";
		...
		cP1.zeta=avgZP; cP2.zeta=avgZP;
		cP1.Rp=avgPR; cP2.Rp=avgPR;
		cP1.Rh=avgSR; cP2.Rh=avgSR;
		if(avgSR-avgPR<=0){cP1.Xsp=0; cP2.Xsp=0;}
		else{cP1.Xsp=avgSR-avgPR; cP2.Xsp=avgSR-avgPR;}
		hamakerConstant=1.8*kb*Z.temperature[i];	//hamakerConstant=calculate_Hamaker_constant(Z.temperature[i],hamakerFldr);
		collisionEfficiency=calc_collision_efficiency(cP1,cP2,hamakerConstant,erValue,kValue,Z.temperature[i],CRITICAL_COAGULATION);
		...
		if(CRITICAL_COAGULATION){Output+="Above CCC: Critical coagulation is occurring\n";}
		else{Output+="Below CCC: Critical coagulation is not occurring\n";}
		Output+=repeatString("-",theLineWidth)+"|\n\n";
